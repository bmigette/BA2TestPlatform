...
NOTE on cache topology (cutover documentation): ``get_ohlcv_data`` now serves
OHLCV from the native PARQUET as_of store via
``ba2_common.core.native_cache.read/write_timeseries`` (the substrate lives in
ba2_common so the ba2_common-defined ``get_ohlcv_data`` can call it without a
ba2_common->ba2_providers edge; ``ba2_providers.cache.native_cache`` re-exports
it). The parquet files live at
``~/Documents/ba2_trade_platform/cache/<ProviderClassName>/<SYMBOL>_<interval>.parquet``;
the first (symbol,interval) request fetches a bounded history once and writes
parquet, and subsequent (symbol,interval,as_of=end_date) requests read a slice
with effective_date<=as_of (no-lookahead, cache-once). So re-sourcing through
ba2_providers yields a SHARED parquet as_of cache for experts + ML. (The legacy
per-class CSV cache helpers remain only for the MarketDataPoint-returning
``get_data`` path.)
"""
