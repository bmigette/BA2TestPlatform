...
class AlphaVantageBaseProvider:
    ...
    def __init__(self, source: str = "dl_forecasting_platform"):
        """
        Initialize the Alpha Vantage base provider.

        Args:
            source: Source identifier for API tracking (e.g., 'dl_forecasting_platform')
        """
        self.source = source
...
def get_api_key() -> str:
    ...
    if not api_key:
        raise ValueError("ALPHA_VANTAGE_API_KEY is not set in environment variables (.env file)")
    return api_key
...
def make_api_request(function_name: str, params: dict, source: str = "dl_forecasting_platform") -> dict | str:
    """
